  ...
  //  public List<LoginResponse.ResultBean.CarilerBean> carilerBeans = new ArrayList<>();
    ...
    public List<LoginResponse.ResultBean.CarilerBean>  filterCity( String searchValue){
        ...
        for(LoginResponse.ResultBean.CarilerBean cari : carilerBeans){
            boolean found =  clearedTurkishCharacter(cari.getAd().replaceAll("I","i").replaceAll("İ","i")).toLowerCase().toString().matches("(?i).*" + clearedTurkishCharacter(searchValue.replaceAll("I","i").replaceAll("İ","i")).toLowerCase().toString()+ ".*");
            if(found){
                newCariler.add(cari);
            }
        }
